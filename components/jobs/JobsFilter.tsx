  ...
  return (
    <div className="relative mt-11 flex w-full  justify-between gap-5 max-sm:flex-col sm:items-center">
      ...
      <Select onValueChange={(value) => handleUpdateParams(value)}>
        ...
        text-dark500_light700   line-clamp-1 flex min-h-[56px] items-center gap-3 border
         p-4 sm:max-w-[210px]"
        >
          <Image
        ...
        <SelectContent className="body-semibold background-light800_dark300  text-dark500_light700 max-h-[350px]   max-w-[250px] font-poppins">
          {/* <SelectGroup>
            {countriesList ? (
              countriesList.map((country: Country) => (
                <SelectItem
                  ...
                  className='mt-1 cursor-pointer  gap-1  bg-slate-200 px-4 py-3 '
                >
                  {country.name.common}
                </SelectItem>
              ))
              ...
              countriesList
                ...
                .map((country) => (
                  <SelectItem
                    key={country.name.common}
                    value={country.name.common}
                    className="mt-1 cursor-pointer gap-1 dark:bg-[#1d2127] dark:text-slate-200 bg-slate-200 px-4 py-3"
                  >
                    {country.name.common}
                  </SelectItem>
                ))
