...
const BlogForm = ({ type, mongoUserId, blogDetails }: Props) => {
  const { mode } = useTheme();
  ...
  const parsedBlogDetails =
    blogDetails && JSON.parse(blogDetails || "");
  // @ts-ignore
  const groupedTags = parsedBlogDetails?.tags.map((tag) => tag.name);
    ...
    async function onSubmit(values: z.infer<typeof BlogSchema>) {
      ...
      try {
        if(type === 'Edit') {
          ...
          router.push(`/myblogs/${parsedBlogDetails._id}`);
        } else {
          await createBlog({
            title: values.title,
            content: values.explanation,
      ...
      >
          ...
          render={({ field }) => (
            <FormItem className="flex w-full flex-col">
              <FormLabel className="paragraph-semibold text-dark400_light800">
                Title of the Blog <span className="text-primary-500">*</span>
              </FormLabel>
        ...
        >
          {isSubmitting ? (
            ...
            <>{type === "Edit" ? "Edit Blog" : "Post Your Blog "}</>
          )}
        </Button>
      </form>
    </Form>
